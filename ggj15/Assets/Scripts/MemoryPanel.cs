using UnityEngine;
using System.Collections;
using System;

public class MemoryPanel : Panel
{	
	public Transform m_trigger;
	public GameObject m_prompt;
	
	private float m_distance;
	private TextMesh m_beaconLetter;

	private string m_key;

	private bool m_bDone = false;

	private bool m_bCheckForInput = false;

	protected override void Start() 
	{
		m_key = GetLetter().ToString();

		m_prompt.SetActive( false );

		m_beaconLetter = m_prompt.GetComponent<TextMesh>();
		m_beaconLetter.text = m_key;
	}

	protected override void Update() 
	{
		if( m_bDone ) { return; }

		if( PanelManager.Instance.CurrentPanelIndex != m_index ) { return; }

		m_distance = Vector2.Distance( PlayerController.Instance.transform.position, m_trigger.position );

		m_bCheckForInput = m_distance < 1.5f;

		m_prompt.SetActive( m_bCheckForInput );

		if( m_bCheckForInput )
		{
			if( Input.GetKeyDown( (KeyCode)Enum.Parse( typeof( KeyCode ), m_key ) ) )
			{
				Debug.Log( "KEY DOWN" );

				m_prompt.SetActive( false );

				m_bDone = true;
			}
		}
	}

	public string GetLetter()
	{
		string chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
		return chars[ UnityEngine.Random.Range( 0, chars.Length )].ToString();
	}
}
